...
int main(void)
{
   ...
   double ts, noise[2], signal_t[256], power_n, fade[2], phase_t[128], phase_r[128];
	...
	for(z=0; z<=35; z+=5)
   {
      ...
      for(j=0; j<n_fft; j++)
      {
		...
		for(i=0; i<ofdm_sym_num; i++)
   	{
         ...
         dfour1(&tx_signal[0],n_fft,1);  // IFFT for transmitted signal must be multiplied by n_fft^(-1)
      ...
      ber=fopen("ber_dqpsk_ofdm_fading_128.log","a");
   	records = fopen("records_dqpsk_ofdm_fading_128.log", "a");
